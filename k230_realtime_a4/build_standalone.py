#!/usr/bin/env python3
"""Build the one-file CanMV deployment for fixed manual A4 calibration."""
...
def _config_block(planner, simulator_validation):
    ...
    lines.append("cfg.PLANNER_BACKEND = {!r}".format(planner))
    if planner == "simulator":
        lines.append(
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        default="local",
        help=(
            "local is safe/fail-closed; upstream exposes proposals for "
            "simulation only"
        ),
    )
    ...
    if output is None:
        output = (
            HERE / "k230_realtime_a4_simulator_standalone.py"
            if args.planner == "simulator"
            else OUTPUT
        )
    sections = [
        "#!/usr/bin/env python3\n"
        ...
        _config_block(args.planner, args.simulator_validation),
